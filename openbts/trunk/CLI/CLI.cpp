...
int sendsms2(string IMSI, char * src, string msg, ostream& os);
int sendtoall(int argc, char** argv, ostream& os);
...
int sendsimple(int argc, char** argv, ostream& os)
{
	...
	char cmd[200];
	// check activeuser table for shortcode
	sprintf(cmd,"sqlite3 %s 'select shortcode from activeuser;'", Users_DB);
	...
	for (int i=2; i<argc; i++) rest = rest + argv[i] + " ";
	...
	if (strlen(line) == 0)	
	{
		os << "No shortcode assigned yet. Wait for the admin activation.";
	}
	else
	{
		...
		snprintf(buffer,1499,form,
			IMSI, (unsigned)random(), line,line,sock.port(),(unsigned)random(), IMSI, (unsigned)random(),sock.port(), strlen(txtBuf), txtBuf);
		sock.write(buffer);
...
int sendsms(int argc, char** argv, ostream& os)
{
	...
	else
	{
		...
		GSM::L3CallingPartyBCDNumber(line),
		GSM::Paging,
		txtBuf);
...
int config(int argc, char** argv, ostream& os)
{
	...
	if (!existing) {
		desc = "Defined new config " + (string)argv[1] + " as \"" + val + "\".";
		os << desc << endl;
		// Anything created by the CLI is optional.
		//gConfig.makeOptional(argv[1]);
	} else {
		desc = "Changed " + (string)argv[1] + " to \"" + val + "\".";
		os << desc << endl;
	}
	return SUCCESS;
}
...
int sendsms2(string IMSI, char * src, string msg, ostream& os) {
	...
	Control::TransactionEntry *transaction = new Control::TransactionEntry(
	gConfig.getStr("SIP.Proxy.SMS").c_str(),
	GSM::L3MobileIdentity(IMSI.c_str()),
	NULL,
	GSM::L3CMServiceType::MobileTerminatedShortMessage,
	GSM::L3CallingPartyBCDNumber(src),
	GSM::Paging,
	msg.c_str());
	transaction->messageType("text/plain");
	Control::initiateMTTransaction(transaction,GSM::SDCCHType,30000);

	string desc = "Message sent to IMSI: " + IMSI + " from " + (string)src;
	desc = desc + "; \"" + msg + "\"";
...
// send message to all connected phones
int sendtoall(int argc, char** argv, ostream& os) {
	if (argc<2) return BAD_NUM_ARGS;
	...
	// count all IMSIs in db
	sprintf(cmd,"sqlite3 %s 'select IMSI from TMSI_TABLE;'", TMSI_DB);
	result = popen(cmd,"r");
	char *lines = (char*)malloc(200);
	int ctr = 0;
	while (!feof(result)) {
		if (!fgets(lines, 200, result)) break;
		ctr++;
	}
	pclose(result);
	...
	sprintf(cmd,"sqlite3 %s 'select IMSI from TMSI_TABLE;'", TMSI_DB);
	result = popen(cmd,"r");
	lines = (char*)malloc(200);
	string IMSI[ctr];
	ctr = 0;
	while (!feof(result)) {
		if (!fgets(lines, 200, result)) break;
		lines[strlen(lines)-1] = '\0';
		IMSI[ctr] = (string)lines;
		ctr++;
	}
	pclose(result);

	for (int i=1; i<argc; i++) rest = rest + argv[i] + " ";
	const char *txtBuf = rest.c_str();
	...
	else
	{
		...
		os << "all messages submitted for delivery" << endl;
	}
	free(line);
	return SUCCESS;
}
...
void Parser::addCommands()
{
	...
	addCommand("sendsms", sendsms, "IMSI message... -- send direct SMS to IMSI, addressed from source number from database.");
	addCommand("sendsimple", sendsimple, "IMSI message... -- send SMS to IMSI via SIP interface, addressed from source number from database.");
	...
	addCommand("sendtoall", sendtoall, "-- Send a message to all connected users.");
}
